...
public class Merge<K, V> {
    ...
    public <M> Merge(Iterable<VersionNode<K, V, M>> versions) {
        checkNotNull(versions, "versions");
        Iterator<VersionNode<K, V, M>> iter = versions.iterator();
        ...
        if (!iter.hasNext()) {
            ...
            VersionNode<K, V, M> versionNode = next(iter);
    ...
    private <M> VersionNode<K, V, M> next(Iterator<VersionNode<K, V, M>> iter) {
        VersionNode<K, V, M> versionNode = iter.next();
